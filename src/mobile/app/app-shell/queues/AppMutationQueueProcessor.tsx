...
const INITIAL_RESUME_DELAY_MS = 900;
const ACTIVE_RESUME_DELAY_MS = 1_200;
const SIGNAL_RESUME_DELAY_MS = 350;
const RESUME_JITTER_WINDOW_MS = 320;
...
export function AppMutationQueueProcessor() {
  ...
  useEffect(() => {
    ...
    const scheduleFallbackResume = async () => {
      clearFallbackTimer();
      const stats = await readMutationQueueStats(ownerId).catch(() => null);
    ...
    const resumeQueue = (allowInactive = false) => {
      ...
      clearFallbackTimer();
      clearResumeTimer();
      const resumePromise = readMutationQueueStats(ownerId)
        .then((stats) => {
          logEvent({
            category: "mutation",
            meta: {
        ...
        .finally(async () => {
          ...
          await readMutationQueueStats(ownerId)
            .then((stats) => {
              logEvent({
                category: "mutation",
                meta: {
    ...
    const scheduleResume = (options?: { allowInactive?: boolean; baseDelayMs?: number }) => {
      ...
      const delayMs =
        ...
        getStableQueueJitterMs(`${ownerId}:mutation`, RESUME_JITTER_WINDOW_MS);
    ...
    const unsubscribeSignal = subscribeQueueResumeSignal("mutation", () => {
      void scheduleResume({
        allowInactive: true,
