...
interface UseEventCardUiStateParams {
  resetKey: string;
}

export function useEventCardUiState(params: UseEventCardUiStateParams) {
  ...
  const handleCopyText = async (value: string, field: "location" | "address") => {
    ...
    try {
      ...
      reportContentCardUiError(error, "event-card-copy", { field });
    }
  };
