-- Query intent on Search should span all viewer-visible cards, not only discovery.
-- Blank query keeps discovery behavior; non-empty query searches across followed
-- and unfollowed visible events/albums plus the broader profile search added in
-- 20260422073000.

create or replace function public.search_events_projection_query_all(
  ...
  return (
    ...
    event_rows as (
      select
        ...
        to_jsonb(ev) as item_json
      from public.list_visible_events('all') ev
      cross join resolved_viewer rv
      where (rv.uid is null or ev.club_user_id <> rv.uid)
        and (
          normalized_query = '' or
          (
            ...
            to_tsvector(
              ...
              coalesce(ev.club_username, '') || ' ' ||
              coalesce(ev.club_name, '')
            ) @@ prefix_tsquery
          ) or
          lower(coalesce(ev.title, '')) like prefix_pattern or
          lower(coalesce(ev.location_name, '')) like prefix_pattern or
          lower(coalesce(ev.club_username, '')) like prefix_pattern or
          lower(coalesce(ev.club_name, '')) like prefix_pattern
        )
        and (category_filter is null or ev.category = category_filter)
        and (university_filter is null or ev.university = university_filter)
        and (
          fee_filter is null or fee_filter = '' or
          (fee_filter = 'free' and lower(coalesce(ev.fee_label, '')) like '%ucretsiz%') or
          (fee_filter = 'paid' and lower(coalesce(ev.fee_label, '')) not like '%ucretsiz%')
        )
        and (
          visibility_filter is null or visibility_filter = '' or
          ev.visibility::text = visibility_filter
        )
        and (since is null or ev.created_at >= since)
        and (
          cursor_timestamp is null
          or (
            not oldest_first
            and (
              ev.created_at < cursor_timestamp
              or (ev.created_at = cursor_timestamp and ev.id::text < coalesce(cursor_identity, ''))
            )
          )
          or (
            oldest_first
...
$$;
create or replace function public.search_albums_projection_query_all(
  viewer_id uuid default null,
  query_text text default '',
  university_filter text default null,
  sort_mode text default 'newest',
...
begin
  return (
    ...
    album_rows as (
      ...
      from public.list_visible_albums('feed', null, null) a
      ...
      where ap.deleted_at is null
        ...
        and (university_filter is null or coalesce(a.uploader_university, '') = university_filter)
...
as $$
  select case
    when lower(trim(coalesce(kind_name, 'events'))) = 'events'
      ...
      then public.search_events_projection_query_all(
        viewer_id,
        query_text,
        category_filter,
        university_filter,
    ...
    when lower(trim(coalesce(kind_name, 'events'))) = 'albums'
      and lower(trim(coalesce(query_text, ''))) <> ''
      then public.search_albums_projection_query_all(
        viewer_id,
        query_text,
        university_filter,
        sort_mode,
...
$$;
grant execute on function public.search_events_projection_query_all(uuid, text, text, text, text, text, text, text, integer, timestamptz) to authenticated;
grant execute on function public.search_albums_projection_query_all(uuid, text, text, text, text, integer, timestamptz) to authenticated;
