    ...
    ordered_rows as (
      ...
      where ap.deleted_at is null
        ...
        and (
          ...
          lower(coalesce(a.club_username, '')) like prefix_pattern or
          lower(coalesce(a.club_name, '')) like prefix_pattern
        )
...
do $$
begin
  if to_regprocedure('public.search_results_projection_base(uuid,text,text,text,text,text,text,text,text,integer,timestamptz)') is null then
    alter function public.search_results_projection(uuid, text, text, text, text, text, text, text, text, integer, timestamptz)
      rename to search_results_projection_base;
  end if;
end;
$$;
...
as $$
begin
  if kind_name = 'albums' then
    return public.search_albums_projection_public_profiles(
      viewer_id,
      query_text,
      sort_mode,
      cursor,
  ...
  return public.search_results_projection_base(
    viewer_id,
    kind_name,
    query_text,
    category_filter,
