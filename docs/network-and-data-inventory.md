...
| Mobile push registration   | Expo Notifications plus existing `/push/register` route             | Expo push token, platform, project/environment attribution                                                              | Platform/Expo transport and user JWT to origin                    | Supabase `push_device_tokens`; Expo delivers notifications                                  | Token lifecycle and terminated/background routing require device proof                                  |
...
| Notifications/push                 | Notification records, push device tokens, dispatch queue, delivery tickets/receipts                           | Restricted device/account data                                                    | User RLS plus service-role delivery worker                     | Receipt processor uses a 15-minute eligibility window and 24-hour expiry for pending Expo receipts | Provider token retention/deactivation and device lifecycle test required                       |
...
namespaces. Preview currently contains deliberate `.invalid` Supabase placeholders and cannot be
deployed until an isolated project is entered.
